...
export default function ModeSelector({ 
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    try {
      ...
      } else if (data.type === 'status' && (data.modoActual !== undefined || data.mode !== undefined)) {
    ...
    const connect = () => {
      ...
      const onClose = (event: CloseEvent) => {
        ...
        if (event.code !== 1000 && reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
          ...
          const baseDelay = Math.min(30000, 1000 * Math.pow(2, reconnectAttempts));
  ...
  }, [handleMessage]); // Removed ws from dependencies to prevent reconnection loops
  ...
  const getIndicatorPosition = () => {
    ...
    return {
      ...
      left: `calc(${(activeMode / (MODES.length - 1)) * ((MODES.length - 1) / MODES.length) * 100}% + 0.25rem)`,
    };
  };
    ...
    <div className={`relative ${className}`}>
      <div className="relative z-10 flex rounded-lg bg-gray-800 p-1">
        ...
        {activeMode !== null && (
          <div 
            ...
            style={{
              ...indicatorPosition,
              background: `linear-gradient(135deg, ${modeToGradient(activeMode)})`,
      ...
      <div className="mt-4 h-1.5 rounded-full bg-gray-800 overflow-hidden">
        <div 
          ...
          style={{
            width: activeMode !== null ? `${((activeMode + 1) / MODES.length) * 100}%` : '0%',
            background: `linear-gradient(90deg, ${modeToGradient(0)}, ${modeToGradient(1)}, ${modeToGradient(2)})`,
            opacity: isDisabled ? 0.5 : 1,
          }}
        />
      </div>
...
function modeToGradient(mode: ModeNum | null): string {
  ...
  switch(mode) {
    case 0: return '#3b82f6, #60a5fa';  // Blue for Piloto
    case 1: return '#eab308, #facc15';  // Yellow for Espera
    case 2: return '#22c55e, #4ade80';  // Green for Manual
    default: return '#6b7280, #9ca3af'; // Default gray
  }
}
